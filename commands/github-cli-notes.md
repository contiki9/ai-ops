...
1. `gh issue view <N> --json title,body,state,labels,url`
2. 必要なら `gh issue view <N> --repo <owner>/<repo> --json title,body,state,labels,url`
3. 失敗時は `gh api repos/<owner>/<repo>/issues/<N>` にフォールバック

`gh issue view <N>` のデフォルト表示のみを実行する手順は避けてください。
